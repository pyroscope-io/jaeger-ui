...
import { FitModes } from './FlameGraphComponent/fitMode';

type ViewTypes = 'flamegraph' | 'both' | 'table';
...
function mountFlamebearer(p: { profile?: Profile; flamebearer?: Flamebearer }) {
  if (p.profile && p.flamebearer) {
    console.warn("'profile' and 'flamebearer' properties are mutually exclusible. Preferring profile.");
  }
...
class FlameGraphRenderer extends React.Component<FlamegraphRendererProps, FlamegraphRendererState> {
  ...
  constructor(props: FlamegraphRendererProps) {
    ...
    this.state = {
      ...
      panesOrientation: props.panesOrientation ? props.panesOrientation : 'horizontal',
  ...
  componentDidUpdate(prevProps: FlamegraphRendererProps, prevState: FlamegraphRendererState) {
    ...
    const tablePane = (
      <div
        ...
        className={clsx(
          styles.tablePane,
          {
            [styles.hidden]: !this.state.flamebearer || this.state.flamebearer.names.length <= 1,
          },
          this.state.panesOrientation === 'vertical' ? styles.vertical : styles.horizontal
        )}
      ></div>
    );
    const dataExists =
